...
class Pinky:
    def __init__(self, pac):
        self.pacman = pac
        self.pos = vec(resources.tile_to_pixel(8, 1))
        self.vel = vec(-1, 0)
        ...
        self.dead_count = 0
        self.active = resources.pinkyActive
        self.replay = False
    ...
    def set_path_nodes(self):
        ...
        if self.return_home:
            ...
            if self.chase:
                ...
                pacman_vel_direction = vec(self.pacman.vel.x, self.pacman.vel.y)
                pacman_vel_direction.normalize_ip()
                target = vec(pacman_pos_tile.x + pacman_vel_direction.x * look_ahead, pacman_pos_tile.y +
                             pacman_vel_direction.y * look_ahead)
                while not (0 < target.x < 28 and 0 < target.y < 31 and
                           not resources.originalTiles[int(target.y)][int(target.x)].wall):
                    look_ahead -= 1
                    target = vec(pacman_pos_tile.x + pacman_vel_direction.x * look_ahead, pacman_pos_tile.y +
                                 pacman_vel_direction.y * look_ahead)

                if target.distance_to(pos_tile) < 2:
                    self.ghost_path_nodes.append(PathNode.PathNode(pacman_pos_tile.x, pacman_pos_tile.y))
                else:
                    self.ghost_path_nodes.append(PathNode.PathNode(target.x, target.y))
            else:
                self.ghost_path_nodes.append(PathNode.PathNode(26, 1))
